/*
 * Renders generated stellar terrain with emission-aware spherical limb darkening.
 */

Shader "jcan/Celestial Systems/Celestial Body Stellar"
{
    Properties
    {
        ...
        [Header(Stellar Limb Darkening)]
        _LimbDarkeningStrength ("Strength", Range(0, 1)) = 0.65
        _LimbDarkeningFalloff ("Falloff", Range(0.1, 8)) = 1.5
        _LimbMinimumBrightness ("Minimum Brightness", Range(0, 1)) = 0.15
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half CelestialResolveLimbDarkening(float3 worldPosition)
            {
                float3 radialNormalWS = CelestialSafeNormalize(
                    worldPosition - _PlanetCenterScenePosition.xyz,
                    float3(0.0, 1.0, 0.0));
                float3 viewDirectionWS = CelestialSafeNormalize(
            ...
            void CelestialApplyLightingMode(
                ...
                float3 worldPosition)
            {
                if (_SurfaceLightingMode > 0.5h)
                {
                    surfaceData.emission =
                        emission *
                        CelestialResolveLimbDarkening(worldPosition);
            ...
            half4 CelestialTerrainFragment(CelestialTerrainVaryings input) : SV_Target
            {
                ...
                if (_LayerCount < 0.5h)
                {
                    ...
                    CelestialApplyLightingMode(surfaceData, baseEmission, input.positionWS);
                }
                else
                {
    ...
    SubShader
    {
        ...
        half ResolveLimbDarkening(float3 worldPosition)
        {
            float3 radialNormalWS = CelestialSafeNormalize(
                worldPosition - _PlanetCenterScenePosition.xyz,
                float3(0.0, 1.0, 0.0));
            ...
            float3 worldPosition)
        {
            if (_SurfaceLightingMode > 0.5h)
            {
                output.Emission =
                    emission *
                    ResolveLimbDarkening(worldPosition);
