...
- Uniform-level collider patches throughout that footprint, including cube-face edges and corners. They use the cached terrain triangles without render skirts or morphing. Each physics triangle is checked and oriented toward the body exterior.
...
6. Select the generated Surface object and use **Drop Test Sphere** in the `CelestialSurfaceCollisionRuntime` component's context menu. A 2 m sphere appears about 5 m above sampled terrain, preferably in front of the camera. It starts with the body's measured point velocity, follows subsequent GE-frame translation/rotation acceleration, uses radial test gravity, and requests its own collision coverage. Check that it lands and `Drop touching terrain` becomes true. On steep terrain it may roll.
...
The generated collision component also runs `CelestialSurfaceCollisionDiagnostics` against the production C# geometry code at initialization. This tests triangle queries, outward physics winding under extreme relief, all eight cube corners, and large universe-cell subtraction. **Validate Collision Geometry** reruns it from the component's context menu. It uses synthetic heights and does not regenerate the graph. Its result is separate from the live PhysX raycast probe. The live probe allows back-face query hits solely for diagnostics and casts from beyond the patch's full elevation range, so a false result indicates a genuinely unavailable collider rather than triangle facing or a ray beginning inside extreme relief.

These numeric checks do not replace Unity compilation and Play Mode validation. GE motion, real graph generation, collider cooking, contacts, and repeated origin shifts must be checked in the scene. The drop sphere compensates for the generated body's changing GE reference frame so its local test is not destabilized by unmodelled orbital acceleration. The current implementation provides localized terrain collision and sample APIs; gameplay character movement, long-term landed velocity inheritance, and a general gravity model remain actor responsibilities.
